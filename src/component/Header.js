  ...
  return (
    <header className="flex justify-between items-start p-4 rounded-t-xl w-full max-w-4xl">
      <div className="flex flex-col items-start space-y-1">
        <h1 className="text-xl font-bold font-gloria">{name}</h1>
        <p className="text-sm max-w-[90px] font-gloria">{job}</p> 
      ...
      <div className="text-center self-start">
        <h2 className="text-2xl font-bold font-gloria">Portfolio</h2>
        <p className="text-sm font-gloria">Contrôle de connaissance</p>
      </div>
      <div className="text-right self-start">
        <p className="text-sm font-gloria">{today}</p>
      </div>
    </header>
  );
}
